...
def estep(X: np.ndarray, mixture: GaussianMixture) -> Tuple[np.ndarray, float]:
    ...
    f = np.zeros((n,K), dtype=np.float64)
       ...
       #np Compute the normal matrix: Single loop implementation
    for i in range(n):
...
def mstep(X: np.ndarray, post: np.ndarray, mixture: GaussianMixture,
    ...
    mu_rev[update_indices] = numerator[update_indices] / denominator[
        update_indices]  # Only update where necessary (denom>=1)
